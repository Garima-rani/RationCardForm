...
function ColorsTable({ setTableData, master, headers = [], data = [], role }) {
          ...
          {data.map((item, index) => (
            <tr key={item.id}>
              ...
              {editId !== item.id ? (
                ...
                <>
                  <td> 
                  //idhar sb grid structure hai
                    <Input
                      placeholder={item.colorName || "Color Name"}
                  ...
                  <td>
                    <Input
                      placeholder={item.colorCode || "Color Hex Code"}
                      defaultValue={item.hexCode}
                      {...register("hexCode", { required: true })}
                    />
                  </td>
                  <td>
                    <Input
          ...
          >
            <div className="bg-white max-h-[80vh] overflow-auto min-w-[400px] md:min-w-[60vw] md:min-h-[60vh] flex flex-col gap-5 bg-[whitesmoke] rounded-md p-4">
              ...
              <div className="grid grid-cols-[40%_60%]">
                ...
                <p>{selectedData?.colorName}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Color Hex Code</p>
                <p>{selectedData?.hexCode}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Color Status</p>
                <p>{selectedData?.status || "Active"}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">

...
function CommodityForm({ setTableData, master, headers = [], data = [], role }) {
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item.id}>
              ...
              {editId !== item.id ? (
                <>
                  <td>{item.commodityFormCode}</td>
                  <td>{item.commodityFormName}</td>
                  <td>{item.commodityFormLocalName}</td>
                  <td>{item.commodityFormShortName}</td>
                  <td>{item?.feedBy || "-"}</td>
                  <td>{item?.feedDate || "-"}</td>
                  <td>{item?.remarks || "-"}</td>
                ...
                <>
                  <td>
                    ...
                    />
                    <Input
                      placeholder={item.commodityFormCode || "Commodity Form Code"}
                      defaultValue={item.commodityFormCode}
                      {...register("commodityFormCode", { required: true })}
                    />
                  </td>
                  <td>
                    <Input
                      placeholder={item.commodityFormShortName || "Commodity Form Short Name"}
                      defaultValue={item.commodityFormShortName}
                      {...register("commodityFormShortName", { required: true })}
                    />
                  </td>
                  <td className="w-fit">

...
function CommodityMaster({ setTableData, master, headers = [], data = [], role }) {
  ...
  const editHandler = async (formData) => {
    try {
      ...
      await axios.put(`http://localhost:8080/api/${master}/${editId}`, {
        ...
        commodityLocalName: formData.commodityLocalName,
        commodityShortName : formData.commodityShortName , 
            ...
            <tr key={item.id}>
              ...
              {editId !== item.id ? (
                <>
                  ...
                  <td>{item?.commodityLocalName || "-"}</td>
                  <td>{item?.commodityShortName || "-"}</td>
                  <td>{item?.commodityGroup.groupName}</td>
                  <td>{item?.measurementUnit.measurementUnitName || '-'}</td>
                  <td>{item?.feedBy}</td>
                  <td>{item?.feedDate}</td>
                ...
                <>
                  <td>
                    <Input
                      placeholder={item?.groupName || "Name"}
                  ...
                  <td>
                    <Input
                      placeholder={item?.nameLocal || "Name Local"}
                      defaultValue={item?.nameLocal}
                      {...register("nameLocal")}
                    />
                  </td>
                  <td>
                  ...
                  <td>{item?.feedDate || '-'}</td>
                  <td className="w-fit">{<Select options={[{name:"Active" } , {name : "Inactive"}]} {...register("status")} />}</td>
        ...
        >
          ...
          >
            <div className="bg-white max-h-[80vh] overflow-auto  min-w-[400px] md:min-w-[60vw]  md:min-h-[60vh] flex flex-col gap-5 bg-[whitesmoke] rounded-md p-4 flex flex-col gap-2 w-full max-w-lg">
              <h1>Commodity Master Details</h1>
              <hr></hr>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Commodity Group Name</p>
                <p>{selectedData?.groupName}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Commodity Group Name Local Language </p>
                <p>{selectedData?.nameLocal}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Commodity Group Short Name</p>
                <p>{selectedData?.shortName}</p>
              </div>
              <div className="grid grid-cols-[40%_60%]">
                <p className="font-bold">Commodity Group Status</p>

...
function CropSeasonMaster({ setTableData, master, headers = [], data = [], role }) {
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item?.id || index}>
              ...
              {editId !== item.id ? (
                <>
                  <td>{item?.cropSeasonCode}</td>
                  <td>{item.cropSeasonName}</td>
                  <td>{item.cropSeasonLocalName}</td>
                  <td>{item.cropSeasonShortName}</td>
                  <td>{item.majorCropSeason}</td>
                  <td>{item.feedBy || '-'}</td>
                  <td>{item.feedDate || '-'}</td>
                ...
                <>
                  <td>
                    <Input
                      placeholder={item.cropSeasonCode || "Season Code"}
                      defaultValue={item.cropSeasonCode}
                      {...register("cropSeasonCode", { required: true })}
                    />
                     <Input
                      placeholder={item.cropSeasonName || "Season Name"}
                      defaultValue={item.cropSeasonName}
                      {...register("cropSeasonName", { required: true })}
                    />
                     <Input
                      placeholder={item.cropSeasonLocalName || "Season Name Local Language"}
                      defaultValue={item.cropSeasonLocalName}
                      {...register("cropSeasonLocalName", { required: true })}
                    />
                     <Input
                      placeholder={item.cropSeasonShortName || "Season Short Name"}
                      defaultValue={item.cropSeasonShortName}
                      {...register("cropSeasonShortName", { required: true })}
                    />
                     <Input
                      placeholder={item.majorCropSeason || "Major Crop Season"}
                      defaultValue={item.majorCropSeason}
                      {...register("majorCropSeason", { required: true })}
                    />
                     <Input
                      placeholder={item.cropSeasonCode || "Season Code"}
                      defaultValue={item.cropSeasonCode}
...
export default CropSeasonMaster;
